...
claude_client = anthropic.Anthropic(api_key=CLAUDE_API_KEY)SYSTEM_PROMPT = """Sen ToyPaxta korxonasi uchun kontent yozuvchi yordamchisan.
Namangan shahrida joylashgan, sifatli toypaxta ishlab chiqaruvchi korxona.

Qoidalar:
- Doim o'zbek tilida yoz
- Samimiy, insonday, haqiqiy uslubda yoz
- Reklama emas, do'stona muloqot kabi
- Qisqa va jo'yali (3-5 jumla)
- Oxirida buyurtma uchun chaqiruv
- 3-5 ta mos hashtag qo'sh
- Emoji lar bilan jonli qil
- Mahsulot: toypaxta (o'yinchoq, yostiq, ko'rpa to'ldirish uchun)

Faqat post matnini yoz, boshqa izoh yozma."""
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        "👋 Salom! Men ToyPaxta kontent botiman.\n\n"
    ...
    try:
        message = claude_client.messages.create(
            ...
            messages=[
                {"role": "user", "content": f"Bu surat haqida post yoz. Qo'shimcha ma'lumot: {caption}"}
            ]
        )
...
async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id not in ADMIN_IDS:
        return
    await update.message.reply_text("⏳ Post yozilmoqda...")
    caption = update.message.caption or "Toypaxta mahsuloti video"
    try:
        message = claude_client.messages.create(
            model="claude-sonnet-4-20250514",
            max_tokens=500,
            system=SYSTEM_PROMPT,
            messages=[
                {"role": "user", "content": f"Video uchun post yoz. Mavzu: {caption}"}
            ]
        )
        post_text = message.content[0].text
        context.user_data["pending_post"] = post_text
...
async def tasdiqlash(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not post_text:
        await update.message.reply_text("❌ Joylash uchun hech narsa yo'q.")
        return
    try:
        if photo_id:
...
async def qayta(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        message = claude_client.messages.create(
            ...
            system=SYSTEM_PROMPT,
            messages=[
                {"role": "user", "content": "Boshqacha uslubda yangi post yoz. Toypaxta mahsuloti haqida."}
            ]
        )
